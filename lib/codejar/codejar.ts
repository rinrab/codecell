...
type CodeJar = ReturnType<typeof CodeJar>

function CodeJar(editor: HTMLElement, highlight: (e: HTMLElement, pos?: Position) => void, opt: Partial<Options> = {}) {
  ...
  function handleSelfClosingCharacters(event: KeyboardEvent) {
    ...
    const escapeCharacter = codeBefore.substr(codeBefore.length - 1) === '\\'
      ...
      if (close.indexOf(event.key) != -1 && !escapeCharacter && charAfter === event.key) {
