...
formula.ACCRINT = function (issue, first, settlement, rate, par, frequency, basis) {
    ...
    return par * rate * formula.YEARFRAC(issue, settlement, basis);
};
...
formula.TBILLEQ = function (settlement, maturity, discount) {
    ...
    return (365 * discount) / (360 - discount * formula.DAYS360(settlement, maturity, false));
};
...
formula.TBILLPRICE = function (settlement, maturity, discount) {
    ...
    // Return bond-equivalent yield
    return 100 * (1 - discount * formula.DAYS360(settlement, maturity, false) / 360);
};
...
formula.TBILLYIELD = function (settlement, maturity, price) {
    ...
    // Return bond-equivalent yield
    return (100 - price) * 360 / (price * formula.DAYS360(settlement, maturity, false));
};
...
formula.XIRR = function (values, dates, guess) {
    ...
    var irrResult = function (values, dates, rate) {
        ...
        var result = values[0];
        for (var i = 1; i < values.length; i++) {
            result += values[i] / Math.pow(r, formula.DAYS(dates[i], dates[0]) / 365);
        }
        return result;
    };
    ...
    var irrResultDeriv = function (values, dates, rate) {
        ...
        var result = 0;
        for (var i = 1; i < values.length; i++) {
            var frac = formula.DAYS(dates[i], dates[0]) / 365;
            result -= frac * values[i] / Math.pow(r, frac + 1);
        }
        return result;
    };
...
formula.XNPV = function (rate, values, dates) {
    ...
    var result = 0;
    for (var i = 0; i < values.length; i++) {
        result += values[i] / Math.pow(1 + rate, formula.DAYS(dates[i], dates[0]) / 365);
    }
    return result;
};
